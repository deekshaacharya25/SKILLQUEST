...
function createOverallScoreContainer() {
    ...
    // Check if skillsSection exists
    if (!skillsSection) {
        console.error('Skills section not found in the DOM');
        return; // Exit the function if skills section is not found
    }
    ...
    if (!overallScoreContainer) {
        ...
        // Insert the new container at the beginning of skillsSection
        skillsSection.insertBefore(overallScoreContainer, skillsSection.firstChild);
    }

    return overallScoreContainer; // Return the container for potential further use
}
